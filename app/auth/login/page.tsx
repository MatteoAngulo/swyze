...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setIsLoading(false)
      setErrors({ general: 'Correo o contrasena incorrectos' })
      return
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      setIsLoading(false)
      setErrors({ general: 'Error al iniciar sesion con Google' })
    }
  }
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center p-8">
        ...
        <div className="w-full max-w-md rounded-2xl border border-border bg-surface-1 p-8">
          ...
          <Button
            ...
            className="w-full h-12 border-border bg-surface-2 text-foreground hover:bg-transparent hover:border-white hover:text-white transition-all disabled:opacity-50"
          >
